    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8">
        <section className="space-y-2 text-center">
          ...
          <p className="text-zinc-400">开业倒计时30天 · 共建竣工开业</p>
        </section>

        <Card className="border-amber-900/40 bg-zinc-900/80">
          <CardHeader>
            <CardTitle className="text-amber-400">当前项目状态</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-2 text-sm text-zinc-300 sm:grid-cols-2">
            {project ? (
              <>
                <p>总进度：{displayProgress(project.progress)}%</p>
                <p>安全：{displayProgress(project.safety)}</p>
                <p>资料完整度：{displayProgress(project.dataIntegrity)}</p>
                <p>消防风险：{displayProgress(project.fireRisk)}</p>
              </>
            ) : (
              <p>项目尚未初始化</p>
            )}
        ...
        <Card className="border-zinc-700 bg-zinc-900/80">
          ...
          <CardContent>
            <pre className="whitespace-pre-wrap text-sm text-zinc-300">{report.content.slice(0, 300)}...</pre>
          </CardContent>
        </Card>
