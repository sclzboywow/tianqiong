    ...
    <section className={`${taskHudPanel} relative border-cyan-400/35 bg-cyan-950/20`}>
      ...
      <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_240px] lg:p-5">
        <div className="min-w-0 pl-1">
          <p className="inline-flex items-center gap-2 text-xs font-medium text-cyan-300">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="mb-1 flex items-center gap-2 text-cyan-400/90">
            ...
            <p className="text-xs font-medium">全局调度 / 任务队列</p>
          </div>
          <h1 className="text-xl font-semibold tracking-wide text-cyan-50 lg:text-2xl">任务调度台</h1>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5 lg:gap-1.5">
          <div className={taskHudMetric}>
            ...
            <p className="mt-0.5 truncate text-[13px] font-semibold text-cyan-50">{stageName}</p>
          </div>
          <div className={taskHudMetric}>
            <p className="text-xs text-slate-500">主线卡点</p>
            <p className="mt-0.5 truncate text-[13px] font-semibold text-cyan-100">
              {hud.mainlineBlocker ?? (summary.mainlineCount > 0 ? `${summary.mainlineCount} 项` : "无")}
            </p>
          </div>
          <div className={taskHudMetric}>
            <p className="text-xs text-slate-500">突发风险</p>
            <p className="mt-0.5 text-[13px] font-semibold tabular-nums text-amber-200">
              {summary.emergencyCount}
            </p>
          </div>
          <div className={taskHudMetric}>
            <p className="text-xs text-slate-500">协作待办</p>
            <p className="mt-0.5 text-[13px] font-semibold tabular-nums text-violet-200">
